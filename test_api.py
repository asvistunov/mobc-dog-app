...
for app in app_urls:
    ...
    for url_type in app_urls[app]:
        ...
        testing_image, _ = client.images.build(
            ...
            tag=f'{url_type}_clinic-app-testing',
                buildargs={
                'APP_URL': app_urls[app][url_type] if url_type == 'url' else 'http://clinic-app:80'
            }
        )
    ...
    logger.info(f'Started building app image from {app_urls[app]["github"]}')
    app_image, _ = client.images.build(
        path='docker/app-hosting/',
        buildargs={
            'GITHUB_REPO_URL': app_urls[app]['github']
        },
        tag='clinic-app'
    )
    logger.info(f'Finished building app image')

    logger.info(f'Running app image from {app_urls[app]["github"]}')
    ...
    for image_type in testing_images:
        ...
        try:
            logger.info(f'Started tests for {app} {image_type} image')
            client.containers.run(
                testing_images[image_type].id,
                command=f'pytest /mobc-dog-clinic-testing/api_testing.py --no-header --no-summary  -q --json-report --json-report-file /mobc-dog-clinic-testing/results/{app}/{image_type}-report.json --json-report-omit warnings',
                network=network.id,
